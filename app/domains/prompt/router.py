...
def create_prompt(
    ...
    기존에 활성화돼 있던 다른 모든 템플릿을 자동으로 비활성화한다(활성 템플릿 최대 1개 유지).
...
def update_prompt(
    ...
    수정한 경우) 다른 모든 템플릿을 자동으로 비활성화해 활성 템플릿을 항상 최대 1개로 유지한다.
    """
...
def delete_prompt(prompt_id: int, db: Session = Depends(get_db)) -> None:
    ...
    활성화되지 않으므로, 삭제 후 활성 템플릿이 0개가 되면 POST /api/v1/ai/chat이 503을 반환한다.
    """
    prompt = service.get_prompt_template(db, prompt_id)
    if prompt is None:
        detail = "prompt template not found"
