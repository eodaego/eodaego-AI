...
**Domain-based layout** (`app/domains/<name>/`): each domain owns `router.py` (HTTP layer only — auth dependency + calls into service, no business logic), `service.py` (business logic + DB access via SQLAlchemy `Session`, no repository abstraction), `model.py` (SQLAlchemy ORM), `schema.py` (Pydantic `*Create`/`*Update`/`*Response` DTOs). Cross-domain model/service imports are allowed one-directionally (e.g. `recommendation` importing from `weather`) as long as they don't form a cycle — if the dependency ever needs to go both ways, promote the shared logic to `core` instead. The cycle check is at domain granularity, not module granularity: if A imports anything from B and B imports anything from A (even different files, e.g. A's `service.py` → B's `service.py` and B's `model.py` → A's `model.py`), that counts as a cycle. See `.claude/rules/10-architecture-and-boundaries.md` for the full rule.

**Auth**: every router except `/health` is registered with `dependencies=[Depends(verify_internal_api_key)]` (`app/core/security.py`), which checks the `X-Internal-Api-Key` header against `Settings.internal_api_key` and 401s on mismatch. This is normally the second line of defense — the first is that the container never publishes a host port (see Deployment). **Temporary exception (2026-07-24 – until real-service launch):** while host port 8001 is published for dev convenience (see Deployment), `X-Internal-Api-Key` is effectively the only line of defense.
...
**Deployment**: single Docker container (`eodaego-ai`, internal port 8000), by principle never given a `-p` host-port publish — it only joins the shared bridge network `eodaego-internal`, which is how `eodaego-server` reaches it (by container name). `.github/workflows/python-server-cicd.yml` builds/pushes to DockerHub then SSHes into the NAS to redeploy, creating the `eodaego-internal` network if it doesn't already exist. uvicorn must bind `0.0.0.0` (not `127.0.0.1`) or same-network containers can't reach it.

**Temporary exception (2026-07-24 – until real-service launch):** for local/dev convenience, the container currently publishes host port 8001 (`-p 8001:8000`) instead of staying fully unbound. This must be reverted to no host-port publish before the real service goes live.
