...
- 성능 / 보안 / 유지보수 관점 체크리스트: 모든 신규 엔드포인트가 `/api/v1` 프리픽스와 공통 에러 포맷을 따르는지, Docker 배포 스크립트에 `-p` 옵션이 추가되지 않았는지.
- 리뷰에서 block 걸어야 하는 기준: 인증 dependency 누락, 호스트 포트 바인딩 추가, import 시점 동기 외부 다운로드 라이브러리 도입, 테스트 코드 추가(정책 위반 — 별도 논의 없이는 금지), git force성 명령 사용(아래 참고).
