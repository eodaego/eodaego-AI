...
- **`eodaego-server` 형제 디렉토리(`/Users/chuseok22/Workspace/projects/eodaego/eodaego-server`) READ-ONLY 절대 규칙**: 이 레포 세션에서 `eodaego-server` 코드를 조사·참고 목적으로 읽는 것은 허용되나, 그 디렉토리 안의 어떤 파일도 생성·수정·삭제해서는 안 된다(Write/Edit/Bash를 통한 파일 조작, git 커밋 등 전부 금지). BE 관련 변경이 필요하다고 판단되면 코드를 직접 고치지 말고 무엇을 바꿔야 하는지 사용자에게 정리해서 보고한다 — 실제 구현은 `eodaego-server` 세션에서 별도로 진행한다.
